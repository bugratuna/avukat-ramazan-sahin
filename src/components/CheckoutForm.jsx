  ...
  if (cartItems.length === 0) {
    return (
      <div id="checkout-section" className="bg-white p-8 rounded-2xl shadow-sm text-center border border-rose-100">
        <p className="text-gray-500">Sepetiniz boş. Yukarıdan lezzetli yemekler ekleyin! 🧁</p>
      </div>
    );
  }
  ...
  return (
    <div id="checkout-section" className="bg-white p-8 rounded-2xl shadow-lg border border-rose-100">
      ...
      <div className="mb-8 bg-rose-50 p-4 rounded-xl">
        <h3 className="font-semibold text-gray-700 mb-3">Sipariş Özeti</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Meryem Tuna"
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Adresiniz</label>
          <textarea
            name="address"
            required
        ...
        >
          <Send size={20} />
            WhatsApp Aracığıyla Sipariş Veriniz
        </button>
      </form>
    </div>
  );
